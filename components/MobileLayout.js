    ...
    <div className={styles.mobileContainer}>
      ...
      <main style={{ flexGrow: 1 }}>
        ...
        <section className={styles.section}>
          <h2 className={styles.sectionTitle}>AI Recruiter</h2>
          <div className={`${styles.agentCard} glass-card`}>
            {!fitData && !isLoading && (
              <div className={styles.inputForm}>
                <div className={styles.agentHeader}>
                  <Sparkles size={16} className={styles.reportIconPurple} />
                  <h3 className={styles.agentTitle}>Tailor Anand's Resume</h3>
                </div>
                <p className={styles.agentDesc}>
                  Paste a vacancy description below to analyze skills alignment and download a tailored resume.
                </p>
                <textarea
                  className={styles.textarea}
                  placeholder="We are looking for a Principal Data Engineer with experience in Snowflake migrations..."
                  value={jobDescription}
                  onChange={(e) => setJobDescription(e.target.value)}
                />
                {error && <div className={styles.errorMsg}><AlertTriangle size={14} />{error}</div>}
                <button 
                  className="btn btn-primary styles.agentBtn" 
            ...
            {isLoading && (
              <div className={styles.loadingBox}>
                ...
                <p style={{ fontWeight: 600, fontSize: "0.95rem" }}>Analyzing Vacancy...</p>
                <div className={styles.terminal}>
                  <div className={styles.terminalHeader}>agent_run.log</div>
                  <div className={styles.terminalBody}>
                    ...
                    {loadingStep < loadingMessages.length - 1 && (
                      <div className={`${styles.terminalLine} ${styles.tPulse}`}>
                        <span className={styles.tYellow}>$</span> <span className={styles.tCursor}>_</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className={styles.reportHeader}>
                  <div className={styles.gaugeOuter}>
                    <div className={styles.gaugeInner}>
                      ...
                      <span className={styles.gaugeLabel}>Fit</span>
                    </div>
                  </div>
                  <div>
                    <h3 className={styles.reportTitle}>Fit Report</h3>
                    <p className={styles.reportDesc}>Tailored resume has been customized to emphasize these alignments.</p>
                  </div>
                </div>

                <div className={styles.reportGrid}>
                  <div className={styles.reportCard}>
                    <h4 className={styles.reportIconGreen}><CheckCircle size={14} />Alignments</h4>
                    <ul>
                      {fitData.strengths.slice(0, 3).map((str, i) => <li key={i}>{str}</li>)}
                    </ul>
                  </div>
                  <div className={styles.reportCard}>
                    <h4 className={styles.reportIconPurple}><Sparkles size={14} />Growth Areas</h4>
                    <ul>
                      {fitData.growth.slice(0, 3).map((gro, i) => <li key={i}>{gro}</li>)}
                    </ul>
                  </div>
                </div>

                <button className="btn btn-primary" onClick={openTailoredResume} style={{ width: "100%", justifyContent: "center" }}>
                  Download Tailored Resume
                </button>
                <button className="btn btn-secondary" onClick={handleReset} style={{ width: "100%", justifyContent: "center", borderColor: "#444" }}>
                  Start Over
                </button>
              </div>
            )}
